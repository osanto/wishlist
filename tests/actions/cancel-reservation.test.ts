...
import { cancelReservationAction } from "@/app/actions/items";
...
describe("cancelReservationAction", () => {
  ...
  it("should successfully cancel a reservation", async () => {
    ...
    // Mock: Check item exists and is reserved by this user
    ...
    // Mock: Update item to unreserved
    ...
    expect(result.data?.reserved_by_token).toBeNull();
  ...
  it("should return error when item is not reserved", async () => {
    // Mock: Validate guest token
    const mockFrom = vi.fn().mockReturnValue({
      select: vi.fn().mockReturnValue({
        eq: vi.fn().mockReturnValue({
    ...
    expect(result.error).toBe("Item is not reserved");
    expect(result.data).toBeUndefined();
    expect(revalidatePath).not.toHaveBeenCalled();
  });

  it("should return error when reservation token does not match", async () => {
    // Mock: Validate guest token
    const mockFrom = vi.fn().mockReturnValue({
      select: vi.fn().mockReturnValue({
        eq: vi.fn().mockReturnValue({
    ...
    // Mock: Item is reserved by different user
    const mockFromItem = vi.fn().mockReturnValue({
      select: vi.fn().mockReturnValue({
        eq: vi.fn().mockReturnValue({
          single: vi.fn().mockResolvedValue({
            data: {
              ...
              wishlist_id: mockWishlistId,
              is_reserved: true,
              reserved_by_token: "different-reservation-token",
            },
            error: null,
          }),
        }),
    ...
    expect(result.error).toBe("You can only cancel your own reservations");
    expect(result.data).toBeUndefined();
    expect(revalidatePath).not.toHaveBeenCalled();
  });
