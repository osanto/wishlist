...
describe('getWishlistByAdminToken', () => {
  ...
  it('should return wishlist when valid admin token is provided', async () => {
    ...
    const mockChain = supabase.from('wishlist').select('*').eq('admin_token', mockAdminToken);
  ...
  it('should return null when invalid admin token is provided', async () => {
    ...
    // Get the mocked chain
    const mockChain = supabase.from('wishlist').select('*').eq('admin_token', invalidToken);
  ...
  it('should return null when database error occurs', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      ...
      { message: errorMessage, code: 'CONNECTION_ERROR' }
    );
