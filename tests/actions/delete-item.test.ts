...
describe("deleteItemAction", () => {
    ...
    mockFrom.mockReturnValueOnce({
      ...
      eq: vi.fn((col: string, val: string) => {
        ...
        return {
          ...
          single: vi
            .fn()
            .mockResolvedValue({ data: { id: mockItemId }, error: null }),
        };
      }),
    });
  ...
  it("should return error when admin token is invalid", async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "Error validating admin token for delete:",
  ...
  it("should return error when item does not belong to the wishlist", async () => {
    ...
    expect(result.error).toBe(
      "Item not found or does not belong to this wishlist"
    );
    expect(result.data).toBeUndefined();
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "Error finding item for delete:",
  ...
  it("should handle database errors gracefully during item deletion", async () => {
    ...
    expect(result.data).toBeUndefined();
    expect(consoleErrorSpy).toHaveBeenCalledWith("Error deleting item:", {
