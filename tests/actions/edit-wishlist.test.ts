...
describe('editWishlistAction', () => {
  ...
  it('should return error when admin token is invalid', async () => {
    ...
    expect(result.data).toBeUndefined();
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error validating admin token for edit:', { message: 'Not found', code: '404' });
  });
  ...
  it('should handle database errors gracefully during update', async () => {
    ...
    mockFrom.mockReturnValueOnce({
      ...
      eq: vi.fn().mockResolvedValue({ error: { message: 'DB update failed', code: 'DB_ERROR' } }),
    });
    ...
    expect(result.data).toBeUndefined();
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error updating wishlist:', { message: 'DB update failed', code: 'DB_ERROR' });
  });
  ...
  it('should convert empty string description to null', async () => {
    ...
    const result = await editWishlistAction(mockAdminToken, dataWithEmptyDescription);
  ...
  it('should handle undefined description', async () => {
    ...
    const result = await editWishlistAction(mockAdminToken, dataWithoutDescription);
